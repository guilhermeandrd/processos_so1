...
//versão com 6 produtore e 2 consumidores 
...
#include <iomanip>
using namespace std;
const int tamanho_buffer= 8; //capacidade do buffer
const int num_produtores = 6; // nùmero de threads de caixa
const int num_consumidores = 2; // número de threads gerentes
...
atomic<int> num_produtores_ativos(0);//contar produtores ativos
atomic<int> total_vendas_geradas(0);
atomic<int> total_vendas_esperadas_por_consumidor(0);
...
void* consumidor(void* args) {
    ...
    while (true) {
        ...
        cv_consumidor.wait(lock, [&] {
            return buffer_vendas.size() == tamanho_buffer || 
                   (num_produtores_ativos == 0 && total_vendas_geradas == total_vendas_esperadas_por_consumidor);
        });

        if (buffer_vendas.empty() && num_produtores_ativos == 0 && total_vendas_geradas == total_vendas_esperadas_por_consumidor) {
            lock.unlock();
            break; 
        }
        ...
        buffer_vendas.clear();
        total_vendas_esperadas_por_consumidor += itens_processados_nesta_rodada;
...
int main(void) {
    ...
    vector<thread> threads_produtoras;
    vector<thread> threads_consumidoras;
    ...
    // Cria as threads consumidoras (gerentes)
    for (int i = 0; i < num_consumidores; ++i) {
        threads_consumidoras.emplace_back(consumidor, static_cast<void*>(NULL));
    }
    ...
    for (thread& t : threads_consumidoras) {
    if (t.joinable()) {
        t.join();
    }
}
...
//em geral a segunda versão só se torna um pouco mais complexa pela existencia de dois consumidores então a criação das threads consimidores e a forma de percorrer para ver se são joinable ou não se tornam muito parecidas como é feito com a das produtoras
//sendo criado um vetor para elas, a condição de parada e notificação também tiveram que ser modificadas para que funcionasse para dois consumidores por isso  a criação das variaveis atomicas total de vendas geradas e total de vendas esperadas para garantir que os produtores não parassem antes da hora
